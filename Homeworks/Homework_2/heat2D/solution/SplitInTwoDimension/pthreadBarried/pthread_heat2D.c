...
#define NXPROB 600                       // grid num in x direction
#define NYPROB 600                       // grid num in y direction
...
pthread_barrier_t barrier;          // Pthread type barrier



// Mutex Worker Threads
//--------------------------------------------------------------------------
void *mutex_worker(void *mydata)    // efficiency is really lower than OpenMP !!!!!!
{
  ...
  // time resolution steps
  printf("thread id: %ld, stating computing step...\n",pMyStatus->tid);  
  for (t_step = 0; t_step <= parms.nts; t_step++)
  {    
    ...
    //------------------------------------------------------------------------------------------------------
    // part II of super computing step: Barried and Sychronization of worker threads
    if (pMyStatus->id == 0)
    {   
       temp = u1;
       u1 = u2;
