...
// global vars
long int n = 100;                              // how many prime number
long int *vecPrime, nPrime, maximum; 


// worker's status vars
struct WorkerStatus {
       pthread_t  id;
       long int   *vecPrime;
       int        nPrime;
...
void serial_prime(long int arg) {
    ...
    while (lbound<arg) {
         ...
         for ( i=lbound; i<ubound; i+=2 ) {
            ...
            if ( nPrime==maximum ) {                                       // alloc memory for new prime number 
               maximum += BLOCK_SIZE;
               temp = vecPrime;
               vecPrime = (long int*) malloc(maximum*sizeof(long int));
...
// Modified code
//-------------------------------------------------     
// These vairbales are for bucket sort
     int *nIndexBucket;
     long int bucketHead;
     ...
     long int *listHead;
     int  *listHeadIndex;        
// These vaible are for sort all elements algorithm
     long int  *temp, bucketShift;
     int index;
//--------------------------------------------------
     ...
     while (lbound<arg) 
     {
         ...
         totalThread = 0;
         pthread_cond_broadcast(&cMaster);          // pthread condition broadcast of master thread    
         while ( totalThread!=thread_num ) pthread_cond_wait(&cWorker, &mtx);


// then we sort all elements in these buckets         
         for (i=0; i<thread_num; i++) {              // Dynamics data store in vecPrime
...
//         qsort(vecPrime, nPrime, sizeof(long int), cmpLongInt);        // quickly sort algorithm
...
// list head record 
   ...
   *nIndexBucket = 0;     
   ...
   {  
      ...
      bucketHead = *(threads[i].vecPrime + i)   ;               // find the first element in every bucket.
...
// first we need sort these bucket
  for (i = 0; i < *nIndexBucket;i++)
  {
...
int main(int argc, char* argv[] ){
    ...
    printf("serial: found %d primes  cost = %15.10f \n", nPrime, serial_cost);
    ...
    printf("mtx   : found %d primes  cost = %15.10f    speedup = %f \n", nPrime, mtx_cost, serial_cost/mtx_cost);
    ...
    printf("atomic: found %d primes  cost = %15.10f    speedup = %f \n", nPrime, atomic_cost, serial_cost/atomic_cost);
    ...
    printf("dup   : found %d primes  cost = %15.10f    speedup = %f \n", nPrime, dup_cost, serial_cost/dup_cost);
