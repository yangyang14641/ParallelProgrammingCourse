...
void *mtx_worker(void *threadid) {
     ...
     while(lbound<MAXIMUM) {
          pthread_cond_wait(&cMaster, &mtx);                                       // let master thread wait 
          ...
          while ( true ) {
          	pthread_mutex_lock(&mtx);
                loc_lbound = lbound;
          	lbound += task_size;
                printf("Task size in thread %ld is %ld\n",tid,task_size);
//          	loc_lbound = lbound;                                               // There is a problem in original sorce code
          	...
          	for ( i=loc_lbound; i<loc_ubound; i+=2 ) {                         // here is same with serial algorithm
          	    while ( i>vecPrime[j-1]*vecPrime[j-1] ) j++;
          	    for ( k=1; k<j; k++ ) if ( i%vecPrime[k] == 0 ) break;
          	    if ( k<j ) continue;
          	    pMyStatus->vecPrime[pMyStatus->nPrime]=i;                      // find prime number
          	    pMyStatus->nPrime++;                                           // prim number counter ++
          	    if ( pMyStatus->nPrime == pMyStatus->maximum ) {               // alloc memory to expand array prime
...
//  Print the prime in selected thread
//-----------------------------------------------------------
/*    if (tid == 0)
    {  
       printf("Thread ID = %ld\n",tid);
       printf("n_prime = %ld\n",pMyStatus->nPrime);
       for (i = 0; i < pMyStatus->maximum; i++) 
         printf("No. %ld, %ld\n",i+1,pMyStatus->vecPrime[i]); 
    }
    printf("\n",tid);
...
//------------------------------------------------------------
     free(pMyStatus->vecPrime);   // mustn't free it when using bucket sort algorithm
    return (void*)0;
}
...
void pthread_prime_mtx(long int arg) {
     ...
     for(i=0; i<thread_num; i++) pthread_create(&(threads[i].id), NULL, mtx_worker, (void *)i);  // create threads
     pthread_mutex_lock(&mtx);
     while ( totalThread!=thread_num ) pthread_cond_wait(&cWorker, &mtx);       // barrier worker threads wait for worker threads

     
     while (lbound<arg) {
         ubound = vecPrime[nPrime-1]*vecPrime[nPrime-1];                            // parallel task division
         if ( ubound<0 || ubound>arg ) ubound = arg;
         ...
         printf("Task size in main thread is %ld\n",task_size);
         totalThread = 0;
         pthread_cond_broadcast(&cMaster);                // pthread condition broadcast of master thread state to worker threads   
         while ( totalThread!=thread_num ) pthread_cond_wait(&cWorker, &mtx);       // barried the master thread

// this part is to sort after every thread finished 
         
          for (i=0; i<thread_num; i++) {
             if ( threads[i].nPrime==0 ) continue;   // this thread didn't find any prime
         ...
         // quickly sort algorithm in fact this algorithm is not necessary at all
